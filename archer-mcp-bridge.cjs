...
/**
 * Start the Archer MCP server with stdio transport
 */
function startMcpServer() {
  const serverPath = path.join(__dirname, 'archer-mcp-server/dist/index.js');
  console.log(`📡 Starting Archer MCP server: ${serverPath}`);
  
  mcpServer = spawn('node', [serverPath], {
    stdio: ['pipe', 'pipe', 'pipe'],
    cwd: path.join(__dirname, 'archer-mcp-server'),
    env: {
      ...process.env,
...
app.get('/tools', async (req, res) => {
  try {
    console.log('🔧 GET /tools - Fetching available tools from Archer MCP server...');
    
    const result = await callMcpMethod('tools/list');
    
    // Transform Archer MCP response to match expected format
    const tools = result.tools || [];
    
    console.log(`✅ Returning ${tools.length} tools from Archer MCP server`);
    res.json({ tools });
...
app.post('/tools/:toolId/call', async (req, res) => {
  try {
    ...
    console.log(`🔧 POST /tools/${toolId}/call - Executing tool via Archer MCP server...`);
...
app.listen(PORT, () => {
  ...
  console.log(`📡 Bridging to Archer MCP server with proper authentication`);
  startMcpServer();
});
