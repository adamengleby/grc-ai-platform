...
import { ArcherCredentials } from '@/lib/backendCredentialsApi';
...
export class TenantScopedMcpService {
  ...
  /**
   * Get tenant-specific MCP server configurations with consolidated storage
   */
  async getEnabledServers(): Promise<TenantMcpServerConfiguration[]> {
    ...
    const consolidatedKey = `tenant_mcp_servers_${this.tenantId}`;
    let stored = localStorage.getItem(consolidatedKey);
    
    // Fallback to legacy storage if consolidated doesn't exist
    if (!stored) {
      const legacyKey = `mcp_servers_${this.tenantId}`;
      ...
      if (stored) {
        try {
          ...
          localStorage.setItem(consolidatedKey, stored);
          localStorage.removeItem(legacyKey);
          console.log(`[MCP Service] Migrated MCP server config from legacy storage for tenant ${this.tenantId}`);
        } catch (error) {
          console.error('[MCP Service] Failed to migrate legacy MCP config:', error);
        }
      }
    }
  ...
  /**
   * Enable an MCP server for the tenant with consolidated storage
   */
  async enableServer(
    serverId: string,
    connectionId: string,
    ...
    // 5. Store with consolidated tenant isolation
    ...
    // Use consolidated storage key
    const consolidatedStorageKey = `tenant_mcp_servers_${this.tenantId}`;
    localStorage.setItem(consolidatedStorageKey, JSON.stringify(updatedServers));
    
    // Clean up old storage locations
    this.cleanupLegacyMcpStorage();
  ...
  /**
   * Clean up legacy MCP server storage locations to prevent drift
   */
  private cleanupLegacyMcpStorage(): void {
    try {
      const legacyKeys = [
        `mcp_servers_${this.tenantId}`,
        `agent_mcp_servers_${this.tenantId}`,
        `mcp_tools_${this.tenantId}` // Also clean up old tool storage
      ];
      
      legacyKeys.forEach(key => {
        if (localStorage.getItem(key)) {
          localStorage.removeItem(key);
          console.log(`[MCP Service] Cleaned up legacy storage: ${key}`);
        }
      });
    } catch (error) {
      console.warn('[MCP Service] Failed to cleanup legacy storage:', error);
    }
  }

  /**
   * Get comprehensive MCP configuration for debugging
   */
  async getMcpConfigurationSummary(): Promise<{
    tenantId: string;
    ...
    for (const server of servers) {
      try {
        ...
        serverDetails.push({
          id: server.id,
          name: server.name || server.serverId || 'Unknown Server',
          status: server.healthStatus || 'unknown',
          toolCount: tools.length,
          lastUsed: server.usage?.lastUsed
        });
    ...
    const possibleKeys = [
      `tenant_mcp_servers_${this.tenantId}`,
      `mcp_servers_${this.tenantId}`,
      `mcp_tools_${this.tenantId}`
    ];
    
    const existingKeys = possibleKeys.filter(key => localStorage.getItem(key) !== null);
  ...
  /**
   * Get available tools from ALL enabled MCP servers for a specific agent
   */
  async getAvailableToolsForAgent(agentId: string): Promise<any[]> {
    try {
      // Get agent with full context to determine enabled servers
      const { createAgentService } = await import('./agentService');
      const agentService = createAgentService(this.tenantId);
      const agentContext = await agentService.getAgentWithContext(agentId);
      
      if (!agentContext || !agentContext.agent) {
        throw new Error(`Agent ${agentId} not found`);
      }
      
      const agent = agentContext.agent;
      const enabledServers = agentContext.mcpServers || [];
      
      console.log(`[MCP Service] Getting tools for agent ${agent.name} from ${enabledServers.length} servers`);
      ...
      for (const server of enabledServers) {
        try {
          ...
          const toolsWithServerId = serverTools.map(tool => ({
            ...tool,
            serverId: server.id,
          ...
          console.error(`[MCP Service] Failed to get tools from server ${server.name}:`, error);
          // Continue with other servers instead of failing completely
        }
      }
      
      console.log(`[MCP Service] Retrieved ${allTools.length} tools total for agent ${agent.name}`);
      return allTools;
      
    } catch (error) {
      console.error('[MCP Service] Failed to get tools for agent:', error);
      throw error;
    }
  }

  /**
   * Get available tools from MCP server using standard MCP protocol (legacy method)
   */
  async getAvailableTools(): Promise<any[]> {
    ...
    try {
      ...
      console.log('[MCP Service] Retrieved available tools (legacy):', result.tools?.length || 0);
   ...
   */
  private async getToolsFromServer(server: any): Promise<any[]> {
    const serverUrl = server.endpoint || 'http://localhost:3001';
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Server ${server.name} returned status: ${response.status}`);
      }
      ...
      console.error(`[MCP Service] Failed to connect to server ${server.name}:`, error);
      throw error;
    }
  }
  ...
  /**
   * Call MCP server with tenant-specific connection context and multi-server routing
   * Connects to the appropriate MCP server based on tool and agent configuration
   */
  private async callMcpServer(toolId: string, inputs: any, connectionContext: Record<string, any>): Promise<any> {
    // Default to primary MCP server URL
    let mcpServerUrl = 'http://localhost:3001';
    let selectedServer = null;
    
    // Try to determine the correct server for this tool
    if (inputs.agentId) {
      try {
        const enabledServers = await this.getEnabledServers();
        // For now, use the first enabled server (can be enhanced with tool-server mapping)
        if (enabledServers.length > 0) {
          selectedServer = enabledServers[0];
          mcpServerUrl = selectedServer.connectionMapping?.connectionId ? selectedServer.endpoint || mcpServerUrl : mcpServerUrl;
        }
      } catch (error) {
        console.warn('[MCP Service] Could not determine server for tool, using default');
      }
    }
    
    console.log(`[MCP Service] Calling tool: ${toolId} on server: ${selectedServer?.name || 'default'}`, 
    ...
    try {
      ...
      const result = await response.json();
      console.log(`[MCP Service] Tool ${toolId} executed successfully on server ${selectedServer?.name || 'default'}`);
      
      // Add server information to result
      return {
        ...result,
        serverId: selectedServer?.id || 'default',
        serverName: selectedServer?.name || 'Default Server'
      };
    } catch (error) {
   ...
   */
  private generateConnectionErrorResponse(toolId: string, inputs: any, _connectionContext: Record<string, any>): any {
