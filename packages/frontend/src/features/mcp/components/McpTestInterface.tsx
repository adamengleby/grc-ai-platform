...
async function orchestrateWithAzureOpenAI(
  ...
  try {
    ...
    if (!selectedAgentConfig) {
      ...
      try {
        const workingTenantId = tenant?.id || selectedTenant?.id || 'a1b2c3d4-e5f6-4a8b-9c0d-1e2f3a4b5c6d'; // Use actual tenant
    ...
    try {
      ...
      if (sessionToken) {
        ...
        const hasAccess = isPlatformOwner || userTenant === parsed.tenantId || 
                         userTenant.includes('manufacturing') && parsed.tenantId === 'a1b2c3d4-e5f6-4a8b-9c0d-1e2f3a4b5c6d';
...
*Agent: ${agent.name} (${agent.useCase})*`,
    ...
    const llmResult = await orchestrateWithAzureOpenAI(userQuery, enhancedTenant, availableTools, mcpService, selectedTenant);
    return llmResult;
  };
