...
const privacy_protector_js_1 = require("./privacy-protector.js");
...
class ArcherDataTransformer {
    ...
    constructor(archerClient) {
        this.archerClient = archerClient;
    }
    /**
     * Get field mapping for any Archer application
     */
    async getFieldMapping(applicationName) {
        // Check cache first
     ...
     */
    transformRecords(rawRecords, fieldMapping, options = {}) {
        const { includeEmptyFields = false, formatValues = true, priorityFields = [] } = options;
        // Validate inputs
    ...
    formatFieldValue(alias, value) {
        ...
        if (Array.isArray(value)) {
            if (value.length === 0)
                return null;
            if (value.length === 1)
                return this.formatSingleValue(alias, value[0]);
     ...
     */
    analyzeDataQuality(rawRecords, fieldMapping) {
        // Validate inputs
        if (!Array.isArray(rawRecords)) {
     ...
     */
    getTopFields(fieldStats, metric, limit = 10) {
     ...
     */
    async generateDataSummary(applicationName, rawRecords, options = {}) {
        try {
            ...
            if (transformedRecords.length > 0) {
                ...
                transformedRecords.slice(0, 3).forEach((record, index) => {
                    ...
                    const priorityKeys = keys.filter(key => key.toLowerCase().includes('id') ||
                        key.toLowerCase().includes('name') ||
                        key.toLowerCase().includes('title')).slice(0, 3);
                    const otherKeys = keys.filter(key => !priorityKeys.includes(key)).slice(0, 4);
...
class ArcherAPIClient {
    ...
    applicationCache = [];
    fieldCache = new Map();
    levelCache = []; // NEW: Cache for level mappings
    dataTransformer; // NEW: Data transformer
    privacyProtector; // NEW: Privacy protection
    constructor(config) {
        this.config = config;
        ...
        this.privacyProtector = new privacy_protector_js_1.PrivacyProtector({
    ...
    async login() {
        try {
            ...
            if (response.data.IsSuccessful && response.data.RequestedObject?.SessionToken) {
                ...
                const errorMessages = response.data.ValidationMessages?.map((msg) => msg.MessageKey || msg.Description || 'Unknown error').join(', ') || 'Unknown error';
    ...
    async getContentApiUrlForLevel(levelId) {
    ...
    async findLevelMapping(nameOrAlias) {
        const mappings = await this.getLevelMappings();
        // Try exact match first
        let mapping = mappings.find(m => m.moduleName.toLowerCase() === nameOrAlias.toLowerCase() ||
            m.alias.toLowerCase() === nameOrAlias.toLowerCase());
        if (!mapping) {
            // Try partial match
            mapping = mappings.find(m => m.moduleName.toLowerCase().includes(nameOrAlias.toLowerCase()) ||
        ...
        try {
            ...
            if (response.data && Array.isArray(response.data)) {
                ...
                debugInfo.sampleData = {
                    ...
                    firstFewItems: response.data.slice(0, 5).map((item) => {
    ...
    async getApplications() {
        ...
        try {
            ...
            const uniqueApplications = applications.filter((app, index, self) => index === self.findIndex(a => a.Id === app.Id));
    ...
    async getQuestionnaires() {
        ...
        try {
            ...
            // Remove duplicates based on Id
            const uniqueQuestionnaires = questionnaires.filter((questionnaire, index, self) => index === self.findIndex(q => q.Id === questionnaire.Id));
    ...
    async getQuestionnaireFields(questionnaireIdOrTargetLevelId) {
        await this.ensureValidSession();
        try {
            if (!this.session?.sessionToken) {
                throw new Error('No valid session token');
    ...
    async getApplicationByName(appName) {
        ...
        let app = this.applicationCache.find((application) => {
    ...
    async searchRecords(appName, pageSize = 100, pageNumber = 1, includeFullData = false) {
        await this.ensureValidSession();
        try {
    ...
    async getAllRecordsWithPagination(url) {
        if (!this.session?.sessionToken) {
            throw new Error('No valid session token');
        }
        let allRecords = [];
    ...
    async getPagedRecords(contentApiUrl, pageSize, pageNumber) {
        if (!this.session?.sessionToken) {
            throw new Error('No valid session token');
        }
        try {
    ...
    async getApplicationFields(appName) {
        try {
            const app = await this.getApplicationByName(appName);
            const fields = await this.getModuleFields(app.Id);
            return {
    ...
    async getTopRecords(appName, topN = 10, sortField) {
        try {
    ...
    async analyzeRisks(appName, options = {}) {
        try {
            const { severityThreshold = 'high', includeOnlyActive = true, topN = 50, sortBy = 'risk_score', includeFullData = false } = options;
            ...
            if (includeOnlyActive && riskFields.status) {
                filteredRecords = records.filter((record) => {
                    ...
                    if (!status)
                        return true; // Include if no status field
            ...
            const sortRisks = (risks) => {
                if (sortBy && risks.length > 0) {
                    const sortField = riskFields[sortBy] || sortBy;
    ...
    generateRiskRecommendations(criticalRisks, highRisks, riskFields) {
    ...
    async findRecordById(appName, recordId) {
        try {
            ...
            if (records.length > 0) {
                const sampleRecord = records[0];
                const idFields = Object.keys(sampleRecord).filter(key => key.toLowerCase().includes('_id') || key.toLowerCase() === 'id');
                if (idFields.length > 0) {
                    idField = idFields[0];
                    foundRecord = records.find((record) => record[idField] == recordId);
                    ...
                    if (!foundRecord && searchResults.totalCount > records.length) {
                        ...
                        foundRecord = allRecords.find((record) => record[idField] == recordId);
    ...
    async getRecordStatistics(appName) {
        try {
            ...
            const idFields = fields.filter(key => key && typeof key === 'string' && (key.toLowerCase().includes('_id') || key.toLowerCase() === 'id')) || [];
            const dateFields = fields.filter(key => key && typeof key === 'string' && (key.toLowerCase().includes('date') ||
                key.toLowerCase().includes('created') ||
                key.toLowerCase().includes('modified'))) || [];
            // Safely generate sample record IDs from all records with validation  
            const sampleRecordIds = allRecords.slice(0, 10).map((record) => {
    ...
    // Get all datafeeds
    async getDatafeeds(activeOnly = true) {
        await this.ensureValidSession();
        try {
            if (!this.session?.sessionToken) {
                throw new Error('No valid session token');
    ...
    async getDatafeedHistory(datafeedGuid) {
        await this.ensureValidSession();
        try {
            if (!this.session?.sessionToken) {
                throw new Error('No valid session token');
    ...
    async getDatafeedHistoryMessages(historyId) {
        await this.ensureValidSession();
        try {
            if (!this.session?.sessionToken) {
                throw new Error('No valid session token');
    ...
    async getDatafeedRecentRun(datafeedGuid) {
        await this.ensureValidSession();
        try {
            if (!this.session?.sessionToken) {
                throw new Error('No valid session token');
    ...
    async getSecurityEvents(instanceName, eventType = "all events", eventsForDate) {
        await this.ensureValidSession();
        try {
            if (!this.session?.sessionToken) {
                throw new Error('No valid session token');
    ...
    try {
        switch (name) {
            ...
            case 'get_archer_applications':
                try {
                    ...
                    if (questionnaires.length > 0) {
                        const questionnaireList = questionnaires.map((quest) => {
                            const parts = [`ID: ${quest.Id}`];
                            if (quest.Alias)
                                parts.push(`Alias: ${quest.Alias}`);
                            parts.push(`GUID: ${quest.Guid}`);
                            parts.push(`Status: ACTIVE`);
                            if (quest.TargetLevelId)
                                parts.push(`TargetLevel: ${quest.TargetLevelId}`);
            ...
            case 'search_archer_records':
                try {
                    ...
                    if (searchResults.transformationSummary) {
                        const protectedSummary = archerClient.privacyProtector.protectData(searchResults.transformationSummary, 'transformation_summary');
                    ...
                    if (includeFullData && searchResults.allRecords && searchResults.allRecords.length > 0) {
                        ...
                        const protectedRecords = archerClient.privacyProtector.protectData(recordsToDisplay, 'search_results_full_data');
                        ...
                        if (searchResults.records.length > 0) {
                            ...
                            const protectedPageRecords = archerClient.privacyProtector.protectData(searchResults.records, 'search_results_page_data');
                            protectedPageRecords.forEach((record, index) => {
            ...
            case 'get_top_records':
                try {
                    const args = request.params.arguments;
                    const topResults = await archerClient.getTopRecords(args.applicationName, args.topN || 10, args.sortField);
                    ...
                    if (topResults.transformationSummary) {
                        const protectedSummary = archerClient.privacyProtector.protectData(topResults.transformationSummary, 'top_records_summary');
                        resultText += `${protectedSummary}\n\n`;
                    }
                    // Apply privacy protection to top records
                    const protectedTopRecords = archerClient.privacyProtector.protectData(topResults.records, 'top_records_data');
                    protectedTopRecords.forEach((record, index) => {
                        resultText += `${index + 1}. `;
                        const keys = Object.keys(record);
                        ...
                        const otherFields = keys.filter(key => !key.toLowerCase().includes('_id') &&
                            typeof record[key] !== 'object').slice(0, 2);
            ...
            case 'analyze_risks':
                try {
                    ...
                    if (fields.severity)
                        resultText += `• Severity: ${fields.severity}\n`;
                    if (fields.impact)
                        resultText += `• Impact: ${fields.impact}\n`;
                    if (fields.likelihood)
                        resultText += `• Likelihood: ${fields.likelihood}\n`;
                    if (fields.riskScore)
                        resultText += `• Risk Score: ${fields.riskScore}\n`;
                    if (fields.priority)
                        resultText += `• Priority: ${fields.priority}\n`;
                    if (fields.status)
                        resultText += `• Status: ${fields.status}\n`;
                    if (fields.annualizedLoss)
                        resultText += `• Annualized Loss: ${fields.annualizedLoss}\n`;
                    if (fields.residualRisk)
                        resultText += `• Residual Risk: ${fields.residualRisk}\n`;
                    if (fields.inherentRisk)
                        resultText += `• Inherent Risk: ${fields.inherentRisk}\n`;
                    resultText += '\n';
                    ...
                    if (riskAnalysis.topRisks && riskAnalysis.topRisks.length > 0) {
                        ...
                        resultText += `────────────────────────────────────────\n`;
                        const protectedRisks = archerClient.privacyProtector.protectData(riskAnalysis.topRisks, 'risk_analysis');
                        protectedRisks.forEach((risk, index) => {
                            resultText += `\n${index + 1}. Risk Record:\n`;
            ...
            case 'get_record_statistics':
                try {
                    ...
                    if (stats.transformationSummary) {
                        const protectedSummary = archerClient.privacyProtector.protectData(stats.transformationSummary, 'data_quality_summary');
                        resultText += `${protectedSummary}\n\n`;
                    }
                    if (stats.structure) {
                    ...
                    if (stats.sampleRecordIds && Array.isArray(stats.sampleRecordIds) && stats.sampleRecordIds.length > 0) {
                        ...
                        stats.sampleRecordIds.forEach((id, index) => {
                ...
                try {
                    ...
                    if (messages) {
                        ...
                        // Check for validation messages
                            ...
                            messages.ValidationMessages.forEach((msg, index) => {
            ...
            case 'check_datafeed_health':
                try {
                    ...
                    if (healthReport.datafeeds && healthReport.datafeeds.length > 0) {
                        ...
                        const failed = healthReport.datafeeds.filter((df) => df.healthStatus === 'Failed');
                        const warnings = healthReport.datafeeds.filter((df) => ['Warning', 'Running', 'No Recent Runs', 'Error'].includes(df.healthStatus));
                        const healthy = healthReport.datafeeds.filter((df) => df.healthStatus === 'Healthy');
                        if (failed.length > 0) {
            ...
            case 'get_security_events':
                try {
                    const args = request.params.arguments;
                    const securityResults = await archerClient.getSecurityEvents(args.instanceName, args.eventType || 'all events', args.eventsForDate);
        ...
        throw new types_js_1.McpError(types_js_1.ErrorCode.InternalError, `Error executing tool: ${error.message}`);
    }
});
// Start server
async function main() {
