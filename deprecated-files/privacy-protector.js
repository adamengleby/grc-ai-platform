...
export class PrivacyProtector {
  constructor(config = {}) {
    this.config = {
      ...
      whitelistedFields: ['id', 'type', 'status', 'created_date', 'modified_date'],
      ...config
    };
  ...
  protectErrorData(error) {
    ...
    if (protectedError.response && protectedError.response.data) {
      protectedError.response.data = this.protectData(protectedError.response.data, 'error');
    }
    
    if (protectedError.config && protectedError.config.headers && protectedError.config.headers.Authorization) {
