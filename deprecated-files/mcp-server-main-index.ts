...
// Environment variables loaded by the server/index.ts file
  ...
  transformRecords(rawRecords: any[], fieldMapping: FieldMapping, options: TransformationOptions = {}): TransformedRecord[] {
    ...
    return rawRecords.map(record => {
      ...
      fieldsToProcess.forEach(alias => {
        if (!Object.prototype.hasOwnProperty.call(record, alias)) return;
...
class ArcherAPIClient {
  ...
  async searchRecords(appName: string, pageSize: number = 100, pageNumber: number = 1, includeFullData: boolean = false): Promise<any> {
    ...
    try {
      ...
      const contentApiUrl = app.ValidatedUrl;
      ...
      if (includeFullData) {
        ...
        try {
          ...
          if (pageError.response?.status === 401) {
            console.error(`🚫 ContentAPI access denied (401) for ${app.Name}. User lacks read permissions for this application.`);
            throw new Error(`ContentAPI access denied (401) for ${app.Name}. User lacks read permissions for this application.`);
          } else {
  ...
  async getPagedRecords(contentApiUrl: string, pageSize: number, pageNumber: number): Promise<{records: any[], totalCount: number}> {
    ...
    try {
      ...
      console.error(`🚀 Making ContentAPI request:`);
      console.error(`   URL: ${pageUrl}`);
      console.error(`   Method: GET`);
      console.error(`   Headers: ${JSON.stringify(requestHeaders, null, 2)}`);
      console.error(`   Session Token: ${this.session.sessionToken ? this.session.sessionToken.substring(0, 20) + '...' : 'NONE'}`);
  ...
  async getAllRecordsAtOnce(url: string): Promise<any[]> {
    ...
    console.error(`🚀 Making ContentAPI request (ALL records):`);
    console.error(`   URL: ${url}?$top=all`);
    console.error(`   Method: GET`);
    console.error(`   Headers: ${JSON.stringify(requestHeaders, null, 2)}`);
    console.error(`   Session Token: ${this.session.sessionToken ? this.session.sessionToken.substring(0, 20) + '...' : 'NONE'}`);
  ...
  async analyzeRisks(appName: string, options: {
    ...
    try {
      ...
      const records = includeFullData && searchResults.allRecords ? searchResults.allRecords : searchResults.records;
    ...
    try {
      ...
      for (const df of datafeeds) {
        try {
          ...
          // let status = 'Unknown'; // Unused variable
  ...
  async getSecurityEvents(instanceName: string, eventType: string = "all events", eventsForDate?: string): Promise<any> {
    ...
    try {
      ...
      // If no date specified, get events for last 7 days
      const dates: string[] = [];
      if (eventsForDate) {
        dates.push(eventsForDate);
      } else {
      ...
      console.error(`🔒 Fetching security events for ${dates.length} day(s), type: ${eventType}...`);
      ...
      for (const date of dates) {
        try {
          ...
          console.error(`✅ Retrieved security events for ${date}`);
          
          // Process and format the response
          let dailyEvents: SecurityEvent[] = [];
          let dailyTotal = 0;
          
          if (response.data && response.data.Links) {
            dailyTotal = response.data.Links.length;
            
            if (response.data.RequestedObject && Array.isArray(response.data.RequestedObject)) {
              dailyEvents = response.data.RequestedObject;
            }
          } else if (Array.isArray(response.data)) {
          ...
          // Add date to each event for tracking
          dailyEvents.forEach(event => {
            event.QueryDate = date;
          });
          ...
          dateResults.push({
            ...
            totalReported: dailyTotal
          });
          
        } catch (dateError: any) {
          console.error(`⚠️ Failed to fetch events for ${date}: ${dateError.message}`);
          dateResults.push({
            date: date,
            eventCount: 0,
            totalReported: 0,
      ...
      return {
        instanceName: instanceName,
        eventType: eventType,
        eventsForDate: eventsForDate || 'Last 7 days',
        dateRange: {
  ...
  async getApplicationStats(appName: string): Promise<ApplicationStats> {
    try {
      ...
      console.error('[ArcherAPIClient] Error getting application stats:', error.message);
      throw error;
    }
  }
}
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description: 'Get security events from Archer for the last 7 days (default) or a specific date. Returns comprehensive security event data with daily breakdown.',
        inputSchema: {
          type: 'object',
          properties: {
            instanceName: {
            ...
            eventsForDate: {
              type: 'string',
              description: 'Optional: Specific date for events in YYYY-MM-DD format (e.g., "2025-08-05"). If not provided, returns last 7 days.'
            }
          },
    ...
    switch (name) {
      ...
      case 'get_security_events':
        try {
          ...
          resultText += `Date Range: ${securityResults.eventsForDate}\n`;
          
          // Show date range details if available
          if (securityResults.dateRange) {
            resultText += `Query Period: ${securityResults.dateRange.startDate} to ${securityResults.dateRange.endDate} (${securityResults.dateRange.daysQueried} days)\n`;
          }
          
          resultText += `Total Events Found: ${securityResults.totalEvents}\n`;
          resultText += `Events Retrieved: ${securityResults.events.length}\n`;
          resultText += `Request Time: ${securityResults.metadata.requestTime}\n\n`;
          
          // Show daily breakdown if multi-day query
          if (securityResults.dateBreakdown && securityResults.dateBreakdown.length > 1) {
            resultText += `📊 DAILY BREAKDOWN:\n`;
            resultText += `${'='.repeat(50)}\n`;
            securityResults.dateBreakdown.forEach((day: any) => {
              if (day.error) {
                resultText += `${day.date}: ❌ Error - ${day.error}\n`;
              } else {
                resultText += `${day.date}: ${day.eventCount} events\n`;
              }
            });
            resultText += '\n';
          }
          ...
          if (securityResults.events && securityResults.events.length > 0) {
            resultText += `🔒 RECENT SECURITY EVENTS (Latest ${Math.min(securityResults.events.length, 20)}):\n`;
            resultText += `${'='.repeat(80)}\n\n`;
            
            // Show first 20 events for readability (already sorted by timestamp descending)
            ...
            eventsToShow.forEach((event: SecurityEvent, index: number) => {
              ...
              if (event.QueryDate) {
                resultText += `  Query Date: ${event.QueryDate}\n`;
              }
            ...
            if (securityResults.totalEvents > 20) {
              resultText += `... and ${securityResults.totalEvents - 20} more events across the time period\n`;
              resultText += `\n💡 TIP: Events are sorted by timestamp (most recent first)`;
            }
          } else {
            resultText += `No security events found for ${securityResults.eventsForDate}\n`;
            resultText += `\nTry:\n`;
            resultText += `- Different event type\n`;
            resultText += `- Specific date (format: YYYY-MM-DD)\n`;
      ...
      case 'get_archer_stats':
        try {
          ...
          const stats = await archerClient.getApplicationStats(args.applicationName);
      ...
      case 'analyze_grc_data':
        try {
          ...
          if (args.applicationName) {
            const stats = await archerClient.getApplicationStats(args.applicationName);
            const sampleRecords = await archerClient.searchRecords(args.applicationName, 20, 1);
          ...
          let analysisResult = `# GRC Data Retrieved\n\n`;
          analysisResult += `**Query**: ${args.query}\n`;
          analysisResult += `**Data Context**: ${dataContext}\n\n`;
          analysisResult += `## Available Data:\n\n`;
          
          if (args.applicationName) {
            analysisResult += `✅ Successfully connected to ${args.applicationName} application\n`;
      ...
      case 'generate_insights':
        try {
          ...
          let insightsResult = `# GRC Data Summary\n\n`;
          insightsResult += `**Focus Area**: ${args.focus_area}\n`;
          insightsResult += `**Data Request Type**: ${args.insight_type || 'summary'}\n`;
          ...
          insightsResult += `**Note**: This tool provides data inventory only. For AI-generated insights and analysis based on your specific focus area (${args.focus_area}), please use an LLM service that can process and analyze the available GRC data.\n\n`;
